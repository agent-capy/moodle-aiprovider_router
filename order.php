...
if ($action !== '' && $confirm) {
    ...
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        throw new moodle_exception('order:cannotapply', 'aiprovider_router');
    }
    ...
    redirect(
        $url,
        get_string('order:applied', 'aiprovider_router'),
        null,
...
if ($router === null) {
    echo $OUTPUT->notification(get_string('check:norouter', 'aiprovider_router'), 'info');
...
if ($action !== '') {
    ...
    echo $OUTPUT->heading(get_string('order:confirm:' . $action, 'aiprovider_router'), 3);
    echo $OUTPUT->box(get_string('order:confirm:' . $action . '_help', 'aiprovider_router'));

    echo $OUTPUT->heading(get_string('order:before', 'aiprovider_router'), 4);
    echo order_formatter::render($inspector->get_entries(), $instances, $routerid);
    echo $OUTPUT->heading(get_string('order:after', 'aiprovider_router'), 4);
    ...
    echo $OUTPUT->single_button(
        ...
        get_string('order:apply', 'aiprovider_router'),
        'post',
    );
...
foreach (aiprovider_router_status_checks() as $check) {
...
echo $OUTPUT->heading(get_string('order:current', 'aiprovider_router'), 3);
echo order_formatter::render($inspector->get_entries(), $instances, $routerid);

echo $OUTPUT->heading(get_string('order:actions', 'aiprovider_router'), 3);

$offered = false;
if (!$inspector->is_router_first()) {
    echo $OUTPUT->box(get_string('order:promote_help', 'aiprovider_router'));
    echo $OUTPUT->single_button(
        new moodle_url($url, ['action' => 'promote']),
        get_string('order:promote', 'aiprovider_router'),
        'get',
    );
    $offered = true;
}
if ($inspector->get_stale_entries()) {
    echo $OUTPUT->box(get_string('order:clean_help', 'aiprovider_router'));
    echo $OUTPUT->single_button(
        new moodle_url($url, ['action' => 'clean']),
        get_string('order:clean', 'aiprovider_router'),
        'get',
    );
    $offered = true;
}
if (!$offered) {
    echo $OUTPUT->notification(get_string('order:nothingtodo', 'aiprovider_router'), 'success');
}
