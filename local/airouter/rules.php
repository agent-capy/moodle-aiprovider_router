...
use local_airouter\rule_formatter;
...
if ($action !== '' && $ruleid > 0) {
    ...
    if ($action === 'delete' && $confirm) {
        ...
        redirect(
            $url,
            get_string('rules:deleted', 'local_airouter', $rule->get('name')),
            null,
    ...
    if (in_array($action, ['up', 'down', 'enable', 'disable', 'duplicate'], true)) {
        ...
        $refused = match ($action) {
            ...
            'duplicate' => $repository->duplicate(
                $ruleid,
                get_string('rules:copyof', 'local_airouter', $rule->get('name')),
            ),
        };
...
if ($action === 'delete' && $ruleid > 0 && !$confirm) {
    ...
    echo $OUTPUT->confirm(
        get_string('rules:confirmdelete', 'local_airouter', $rule->get('name')),
...
// Which instances a brought key could actually be put into, and which ones the site's own
// key may not be used at, so that a rule which can never be honoured says so rather than
// quietly never claiming anything.
$settings = new target_settings($DB);
$byokcapable = $settings->get_byok_capable_ids();
$byokonly = $settings->get_byok_only_ids();

echo $OUTPUT->box(get_string('rules:intro', 'local_airouter'));

if (!$rules) {
    echo $OUTPUT->notification(get_string('rules:none', 'local_airouter'), 'info');
} else {
    $table = new html_table();
    $table->head = [
        get_string('rules:priority', 'local_airouter'),
        get_string('rule:name', 'local_airouter'),
        get_string('rule:conditions', 'local_airouter'),
    ...
    foreach ($rules as $id => $rule) {
        ...
        $row->cells = [
            ...
            rule_formatter::target($rule, $targets, $byokcapable, $byokonly),
...
echo $OUTPUT->single_button(
    new moodle_url('/local/airouter/rule.php'),
    get_string('rules:add', 'local_airouter'),
    'get',
);
echo $OUTPUT->single_button(
