...
use local_airouter\form\byok_policy_form;
use local_airouter\form\byok_targets_form;
...
if ($data = $policyform->get_data()) {
    ...
    redirect(
        $url,
        get_string('byok:saved', 'local_airouter'),
        null,
...
if ($unknown) {
    ...
    echo $OUTPUT->notification(
        get_string('eligibility:unknown', 'local_airouter', s(implode(', ', $unknown))),
        'warning',
    );
}
$policyform->display();

echo $OUTPUT->heading(get_string('byok:targets', 'local_airouter'), 3);

if (!$targets) {
    echo $OUTPUT->notification(get_string('defaulttarget:none', 'local_airouter'), 'info');
    echo $OUTPUT->footer();
    die;
}
...
if ($unanswered) {
    echo $OUTPUT->notification(
        get_string('byok:unanswered', 'local_airouter', count($unanswered)),
        'warning',
    );
}
...
if ($held > 0) {
    echo html_writer::div(get_string('byok:held', 'local_airouter', $held), 'text-muted');
}
