    ...
    public static function add_to_form(\MoodleQuickForm $mform): void {
        $group = [
            $mform->createElement('select', 'promptlengthoperator', '', [
                self::OPERATOR_GTE => get_string('condition:promptlength:gte', 'local_airouter'),
                self::OPERATOR_LTE => get_string('condition:promptlength:lte', 'local_airouter'),
            ]),
        ...
        $mform->addHelpButton('promptlengthgroup', 'condition:promptlength', 'local_airouter');
    }
