...
class byok_policy_form extends \moodleform {
    ...
    protected function definition(): void {
        ...
        foreach (eligibility_policy::get_access_options() as $access) {
            $options[$access] = get_string('eligibility:access:' . $access, 'local_airouter');
        }
        $mform->addElement('select', 'access', get_string('eligibility:access', 'local_airouter'), $options);
        $mform->setDefault('access', eligibility_policy::ACCESS_NOBODY);
        $mform->addHelpButton('access', 'eligibility:access', 'local_airouter');

        $matches = [];
        foreach (eligibility_policy::get_match_options() as $match) {
            $matches[$match] = get_string('eligibility:match:' . $match, 'local_airouter');
        }
        $mform->addElement('select', 'match', get_string('eligibility:match', 'local_airouter'), $matches);
        $mform->setDefault('match', eligibility_policy::MATCH_ANY);
        $mform->addHelpButton('match', 'eligibility:match', 'local_airouter');
        $mform->hideIf('match', 'access', 'neq', eligibility_policy::ACCESS_CONDITIONS);

        foreach (registry::get_types() as $type) {
            $class = 'local_airouter\\eligibility\\' . $type;
