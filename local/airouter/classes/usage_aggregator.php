...
class usage_aggregator {
    ...
    public const TABLE = 'local_airouter_daily';
    ...
    public function run(int $now): array {
        ...
        foreach ($days as $day) {
            ...
            set_config(self::LAST_SETTING, $day, 'local_airouter');
        }
    ...
    public function get_days_to_summarise(int $now): array {
        $last = (int) get_config('local_airouter', self::LAST_SETTING);
        if ($last > 0) {
    ...
    public function get_retention_days(): int {
        $configured = get_config('local_airouter', self::RETENTION_SETTING);
    ...
    public function get_summary_retention_days(): int {
        $configured = get_config('local_airouter', self::SUMMARY_RETENTION_SETTING);
        if ($configured === false || $configured === '') {
