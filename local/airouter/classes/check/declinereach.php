...
class declinereach extends base {
    ...
    protected function check_router(): result {
        ...
        if (!$following) {
            ...
            return new result(result::OK, get_string('check:declinereach:alone', 'local_airouter'));
        }
        ...
        if ($this->inspector->get_primary_router()->is_strict_decline()) {
            return new result(
                result::OK,
                get_string('check:declinereach:strict', 'local_airouter', count($following)),
                $details,
            );
        }
        ...
        return new result(
            result::WARNING,
            get_string('check:declinereach:bypassable', 'local_airouter', count($following)),
            $details,
        );
    }
}
