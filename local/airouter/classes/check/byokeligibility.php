...
class byokeligibility extends base {
    ...
    protected function check_router(): result {
        ...
        if ($policy->get_access() !== eligibility_policy::ACCESS_CONDITIONS) {
            ...
            return new result(result::NA, get_string('check:byokeligibility:noconditions', 'local_airouter'));
        }

        $condition = $policy->get_conditions()['profilefield'] ?? null;
        if (!$condition instanceof profilefield || !$condition->is_self_declared()) {
            return new result(result::OK, get_string('check:byokeligibility:ok', 'local_airouter'));
        }
        ...
        return new result(
            $status,
            get_string('check:byokeligibility:selfdeclared', 'local_airouter'),
            get_string(
