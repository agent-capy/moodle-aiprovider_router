...
class staleentries extends base {
    ...
    protected function check_router(): result {
        ...
        if (!$stale) {
            return new result(result::OK, get_string('check:staleentries:ok', 'local_airouter'));
        }

        return new result(
            result::WARNING,
            get_string('check:staleentries:found', 'local_airouter', count($stale)),
            get_string('check:staleentries:found_details', 'local_airouter', implode(', ', $stale)),
        );
    }
}
