...
class budget_notifier {
    ...
    public const TABLE = 'local_airouter_notice';
    ...
    protected const WATCHER_CAPABILITY = [
        ...
        self::KIND_COURSE => 'local/airouter:viewusage',
        self::KIND_USER => 'local/airouter:viewuserusage',
    ];
    ...
    public static function is_enabled(): bool {
        $configured = get_config('local_airouter', self::ENABLED_SETTING);
    ...
    public static function get_share(): int {
        $configured = get_config('local_airouter', self::SHARE_SETTING);
    ...
    protected function post(string $provider, \stdClass $user, string $key, \stdClass $figures): void {
        ...
        $message->component = 'local_airouter';
        $message->name = $provider;
        $message->userfrom = \core_user::get_noreply_user();
        $message->userto = $user;
        $message->subject = get_string('notify:' . $provider . ':subject', 'local_airouter');
        $message->fullmessage = get_string($key, 'local_airouter', $figures);
        $message->fullmessageformat = FORMAT_PLAIN;
        $message->fullmessagehtml = '';
        $message->smallmessage = $message->subject;
        $message->notification = 1;
        $message->contexturl = (new \moodle_url('/local/airouter/usage.php'))->out(false);
        $message->contexturlname = get_string('usage:heading', 'local_airouter');
    ...
    protected function describe(string $kind, int $subjectid): string {
        if ($kind === self::KIND_COURSE) {
            ...
            return $name === false
                ? get_string('report:gonecourse', 'local_airouter', $subjectid)
                : format_string($name);
        }
        ...
        return get_string('notify:subject:site', 'local_airouter');
    }
