...
class teaching extends base {
    ...
    public static function add_to_form(\MoodleQuickForm $mform): void {
        ...
        $mform->addHelpButton('eligibilityroles', 'eligibility:teaching', 'local_airouter');
    }
    ...
    public function get_description(): string {
        ...
        foreach ($this->config['roles'] ?? [] as $roleid) {
            $names[] = $known[(int) $roleid] ?? get_string('eligibility:role:gone', 'local_airouter', $roleid);
        }

        return get_string('eligibility:teaching:described', 'local_airouter', implode(', ', $names));
    }
