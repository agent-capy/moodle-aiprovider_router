...
class cohort extends base {
    ...
    public static function add_to_form(\MoodleQuickForm $mform): void {
        ...
        $mform->addHelpButton('eligibilitycohorts', 'eligibility:cohort', 'local_airouter');
    }
    ...
    public function get_description(): string {
        ...
        foreach ($this->config['cohorts'] ?? [] as $cohortid) {
            ...
            $names[] = $known[(int) $cohortid]
                ?? get_string('eligibility:cohort:gone', 'local_airouter', $cohortid);
        }

        return get_string('eligibility:cohort:described', 'local_airouter', implode(', ', $names));
    }
