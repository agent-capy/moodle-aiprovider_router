...
use local_airouter\eligibility\base as eligibility_base;
use local_airouter\eligibility\registry;
...
class eligibility_policy {
    ...
    public function get_access(): string {
        $access = (string) get_config('local_airouter', self::ACCESS_SETTING);
    ...
    public function get_match(): string {
        $match = (string) get_config('local_airouter', self::MATCH_SETTING);
    ...
    public function get_stored_conditions(): array {
        $stored = json_decode((string) get_config('local_airouter', self::POLICY_SETTING), true);
    ...
    public function save(string $access, array $conditions, string $match = self::MATCH_ANY): void {
        ...
        set_config(self::ACCESS_SETTING, $access, 'local_airouter');
        set_config(self::MATCH_SETTING, $match, 'local_airouter');
        set_config(self::POLICY_SETTING, json_encode($conditions), 'local_airouter');
        self::purge();
    }
    ...
    protected function get_cache(): \core_cache\cache {
        return \core_cache\cache::make('local_airouter', self::CACHE_AREA);
    }
}
