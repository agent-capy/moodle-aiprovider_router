...
class order_formatter {
    ...
    public static function describe_entry(string $entry, array $instances, ?int $routerid): string {
        if ($entry === '') {
            return get_string('order:entry:empty', 'local_airouter');
        }
        if (!isset($instances[$entry])) {
            return get_string('order:entry:stale', 'local_airouter', s($entry));
        }
        ...
        return get_string($key, 'local_airouter', $placeholders);
    }
