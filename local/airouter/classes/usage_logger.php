 ...
 * provider the manager called, which is always local_airouter. Core's own usage
...
class usage_logger {
    ...
    public const TABLE = 'local_airouter_log';
    ...
    public function record(\stdClass $entry, int $images = 0): void {
        try {
            ...
            debugging(
                'local_airouter: could not record usage: ' . get_class($e) . ': ' . $e->getMessage(),
                DEBUG_NORMAL,
            );
        }
    }
