...
final class usage_aggregator_test extends \advanced_testcase {
    ...
    public function test_the_mark_of_what_was_discarded_only_moves_forward(): void {
        set_config(usage_aggregator::HISTORY_SETTING, $this->day(1), 'local_airouter');
        $this->log($this->day(10) + HOURSECS);
        set_config(usage_aggregator::RETENTION_SETTING, 1, 'local_airouter');
        set_config(usage_aggregator::SUMMARY_RETENTION_SETTING, 2, 'local_airouter');
    ...
    public function test_days_with_nothing_in_them_are_passed_over(): void {
        ...
        $this->assertSame(1, $result['rows']);
        $this->assertSame($this->day(1), (int) get_config('local_airouter', usage_aggregator::LAST_SETTING));
    }
    ...
    public function test_the_purge_stops_at_the_last_day_summarised(): void {
        ...
        set_config(usage_aggregator::LAST_SETTING, $this->day(35), 'local_airouter');
    ...
    public function test_summaries_past_their_retention_are_removed(): void {
        set_config(usage_aggregator::RETENTION_SETTING, 30, 'local_airouter');
        set_config(usage_aggregator::SUMMARY_RETENTION_SETTING, 90, 'local_airouter');
        $this->log($this->day(400) + HOURSECS);
        $this->log($this->day(10) + HOURSECS);
    ...
    public function test_a_summary_is_never_removed_while_its_detail_survives(): void {
        ...
        set_config(usage_aggregator::RETENTION_SETTING, 90, 'local_airouter');
    ...
    public function test_keeping_every_detail_row_keeps_every_summary(): void {
        set_config(usage_aggregator::RETENTION_SETTING, 0, 'local_airouter');
        set_config(usage_aggregator::SUMMARY_RETENTION_SETTING, 30, 'local_airouter');
        $this->log($this->day(400) + HOURSECS);
        $this->aggregator->summarise_day($this->day(400));
    ...
    public function test_a_purge_never_takes_history_a_budget_still_needs(): void {
        ...
        (new rule_repository($DB))->save($rule, ['budget' => [
            ...
            'direction' => \local_airouter\condition\budget::DIRECTION_UNDER,
