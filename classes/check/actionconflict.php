...
class actionconflict extends base {
    ...
    protected function check_router(): result {
        ...
        if (!$intercepting) {
            return new result(result::OK, get_string('check:actionconflict:ok', 'aiprovider_router'));
        }
        ...
        return new result(
            result::WARNING,
            get_string('check:actionconflict:found', 'aiprovider_router', count($intercepting)),
            get_string('check:actionconflict:found_details', 'aiprovider_router', implode(', ', $names)),
        );
    }
}
