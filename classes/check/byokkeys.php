...
class byokkeys extends base {
    ...
    protected function check_router(): result {
        ...
        if ($total === 0) {
            ...
            return new result(result::NA, get_string('check:byokkeys:nokeys', 'aiprovider_router'));
        }
        ...
        if ($unreadable === 0) {
            return new result(
                result::OK,
                get_string('check:byokkeys:ok', 'aiprovider_router', $total),
            );
        }
        ...
        return new result(
            result::ERROR,
            get_string('check:byokkeys:unreadable', 'aiprovider_router', [
                'unreadable' => $unreadable,
                'total' => $total,
            ]),
            get_string('check:byokkeys:unreadable_details', 'aiprovider_router'),
        );
    }
}
