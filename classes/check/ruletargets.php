...
class ruletargets extends base {
    ...
    protected function check_router(): result {
        ...
        if (!$broken) {
            return new result(result::OK, get_string('check:ruletargets:ok', 'aiprovider_router'));
        }

        return new result(
            result::WARNING,
            get_string('check:ruletargets:found', 'aiprovider_router', count($broken)),
            get_string('check:ruletargets:found_details', 'aiprovider_router', s(implode(', ', $broken))),
        );
    }
}
