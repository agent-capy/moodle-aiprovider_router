...
class key_form extends \moodleform {
    ...
    protected function definition(): void {
        ...
        $mform->addHelpButton('targetid', 'keys:target', 'aiprovider_router');

        $mform->addElement('passwordunmask', 'secret', get_string('keys:secret', 'aiprovider_router'));
        $mform->setType('secret', PARAM_RAW_TRIMMED);
        $mform->addHelpButton('secret', 'keys:secret', 'aiprovider_router');

        $this->add_action_buttons(false, get_string('keys:save', 'aiprovider_router'));
    }
    ...
    public function validation($data, $files): array {
        ...
        if ((int) ($data['targetid'] ?? 0) <= 0) {
            $errors['targetid'] = get_string('key:error:notarget', 'aiprovider_router');
        }
        if (trim((string) ($data['secret'] ?? '')) === '') {
            $errors['secret'] = get_string('key:error:empty', 'aiprovider_router');
        }
